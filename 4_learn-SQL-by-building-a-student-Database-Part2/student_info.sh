...
PSQL="psql -X --username=jemal.khachidze --dbname=students --no-align --tuples-only -c"
...
echo "$($PSQL "SELECT major FROM majors LEFT JOIN students ON majors.major_id = students.major_id WHERE students.major_id IS NULL OR students.first_name ILIKE '%ma%';")"
